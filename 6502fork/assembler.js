...
function SimulatorWidget(node) {
  ...
  async function initialize() {
    ...
    $node.find('.flashButton').click(function() {
      ...
      console.log(input.files[0] ?? defaultData)
      fr.readAsArrayBuffer(input.files[0] ?? defaultData);
    }).click();

    $node.find('.runButton').click(simulator.runBinary).click();
  ...
  var ROM = new ArrayBuffer(8192),currentBank=0;
  function Memory() {
    ...
    function updateRomBank(){
      currentBank = get(0xfd)
      const bank = new Uint8Array(ROM.slice(currentBank*8192,(currentBank+1)*8192))
      flash(0x1000,Array(8192).fill(0))
      //console.log(ROM.slice(currentBank*8192,(currentBank+1)*8192))
      //console.log(flash(0x1000,bank))
      flash(0x1000,bank)
    }
  ...
  function Simulator() {
    ...
    function runBinary() {
      if (codeRunning) {
        ...
        ui.play();
        codeRunning = true;
        executeId = setInterval(multiExecute, 0);
      }
    }
  ...
  function Assembler() {
    ...
    function loadBinary(arrayBuffer) {
      ...
      ROM = arrayBuffer
      console.log(ROM.byteLength,Math.ceil(ROM.byteLength/8192))
      memory.set(0xfc,Math.ceil(ROM.byteLength/8192))
      memory.set(0xfd,0)
      memory.updateRomBank()
      codeLen = ROM.byteLength;
